...
def checkCollision(movingBall):
    for a in objectBalls:
        if abs(a[0] - movingBall[0]) < 16 and abs(a[1] - movingBall[1]) < 16:
            a[2] = ((movingBall[0] - a[0]) * - 1) / 15
            a[3] = ((movingBall[1] - a[1]) * - 1) / 15
            #movingBall[2] = 0
            #movingBall[3] = 0
            if a[3] == 0:
                a[3] = 1
...
bottomLeft = [0, 800]
bottomRight = [400, 800]
pockets = [topLeft, topRight, centerLeft, centerRight, bottomLeft, bottomRight]
#Game Loop
running = True
while running:
    ...
    pygame.draw.circle(screen, black, (topLeft[0], topLeft[1]), 30)
    pygame.draw.circle(screen, black, (topRight[0], topRight[1]), 30)
    pygame.draw.circle(screen, black, (centerLeft[0], centerLeft[1]), 30)
    pygame.draw.circle(screen, black, (centerRight[0], centerRight[1]), 30)
    pygame.draw.circle(screen, black, (bottomLeft[0], bottomLeft[1]), 30)
    pygame.draw.circle(screen, black, (bottomRight[0], bottomRight[1]), 30)
    ...
    for a in objectBalls:
        if (a[2] > 0.05 or a[2] < -0.05) and (a[3] > 0.05 or a[3] < -0.05):
            ...
            if a[1] >= 793 or a[1] <= 7:
                a[3] = a[3] * -1
            for x in pockets:
                if abs(a[0] - x[0]) < 28 and abs(a[1] - x[1]) < 28:
                    a[0] = 800
                    a[1] = 800
                    objectBalls.remove(a)
                #a[3] = a[3] / 1.2
            a[3] = a[3] * friction
            a[2] = a[2] * friction
            a[0] += a[2]
    ...
    if cueBall[2] > 0 or cueBall[2] < 0 or cueBall[3] > 0 or cueBall[3] < 0:
